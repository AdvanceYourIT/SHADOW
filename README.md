...
- **Policy Override Audit — Export JSON / CSV.** The Overrides report adds **Export JSON** and **Export CSV** buttons next to **Save HTML**. JSON captures the full audit data structure; CSV flattens the audit to one row per diff (scope, entity, organization, category, setting, type, property, diff type, parent value, override value) for quick spreadsheet review.
- **Asset Report — Export CSV.** Next to **Save HTML**, an **Export CSV** button exports the current (filtered) device list with **name, organization, age, serial number, processor and memory** columns for quick spreadsheet review.
- **Script Usage — Export CSV.** Both tabs gain an **Export CSV** button alongside **Download JSON**. The **Unused** tab exports name, ID and language; the **Used** tab exports each script plus reference counts per category (policies, patch automation, scheduled tasks, system tray, dynamic policies, device overrides) and a total.
- **Reporting page buttons stay together.** The **Policy Report**, **Overrides**, and **Asset Report** buttons on the Reporting page now render inside a single shared container anchored to one header, so they line up neatly as a group instead of scattering across the page or being stranded by NinjaOne's SPA re-renders.
- **Policy Override Audit — new controls.** The Overrides report adds an **Open in NinjaOne** button on the base and child policy headers (deep-links straight to the policy editor), a **Hide clean** toggle that collapses away policies and devices with no overrides left after filtering, and **Expand all / Collapse all** buttons. The downloaded standalone HTML report carries the same controls.
- **Script Usage — device-level overrides.** An optional **Include device-level overrides** toggle scans every device's effective policy and picks up automations that were added or changed directly on a device (policy overrides applied at device level) — previously these were invisible to the scan, so a script used only that way could show as unused. Matches appear in a new **Device Overrides** column on the Used tab. Opt-in because it costs one API call per device.
- **Script Usage — Ghost References tab.** A new **Ghost References** tab surfaces references in policies, conditions or tasks that point to a script **id/uid that no longer exists** — typically an old version left behind after a script was deleted and re-created. NinjaOne keeps showing the cached name (so the script *looks* in use), but the current script is genuinely unused. The tab lists the stale name, the dead identifier and exactly where it is referenced; re-selecting the script in the condition/task clears the ghost. JSON and CSV export included.
- **Fixed: Import from Public/Private GitHub settings check.** The import overlay could report "GitHub settings not configured" even when the repository was configured correctly. The GitHub messaging helpers are now shared (single implementation) across the backup, import, and compare modules, fixing the broken settings retrieval and the private-repo token check.
- **Fixed: Script Usage now degrades gracefully for limited-access technicians.** A single source the scan could not load (for example **system tray configurations**) used to abort the entire overlay. Policies, scheduled tasks, system tray and dynamic policies now each fail softly — the report renders from whatever did load, with a clear notice at the top listing the sources that could not be loaded. A genuine re-authentication requirement (MFA) still stops the scan.
- **Fixed: automatic backup no longer runs concurrently.** When the last backup was older than the configured age, opening more browser tabs while a backup was still running could kick off several full backups at once. A shared, cross-tab **in-progress lock** now guarantees only one backup runs at a time — extra tabs and a manual **Backup Now** are skipped (with a clear message) until it completes, and a stale lock left behind by a closed/crashed tab expires automatically.
- **Fixed: automatic backup no longer runs on the login screen.** Startup auto-backup used to fire on every page load, including while you were still on the NinjaOne login screen, where it always failed and popped a backup-error alert. It now confirms an active session first and silently skips the run until you're actually logged in.
- **Fixed: Custom Field Manager Edit mode applies detail and permission changes reliably.** Editing descriptions, footer/tooltip text, or Script/Technician/API permissions now correctly enables **Apply**, and update requests strip NinjaOne read-only/UI-only fields before saving so the SWB endpoint no longer rejects payloads with 400 errors such as `customizeForEndUser`.
...
- **New Bulk Set Values for organization fields.** A **Bulk Set Values** button on the Organization Custom Fields page sets organization custom field values across all or selected organizations at once — either by entering a value manually (type-aware input for text, checkbox, number, date, dropdown, and multi-select fields) or by copying field value(s) from a chosen source organization onto the targets. It reads each org's current values and writes back only the chosen field(s) — every other value is left untouched — and fields the source organization has not set are skipped so targets are never blanked out.
